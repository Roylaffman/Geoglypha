...
# Resolve shared helpers from quarto-gis
...
from map_builder import create_base_map, finalize_map

# farm.geojson — personal sighting, lives in the old analysis folder
FARM_PATH = QUARTO_GIS / "analysis" / "farm.geojson"
```
...
In 2023 and 2024, the U.S. Fish and Wildlife Service made it federal: distinct population segments of *Rana boylii* are now listed as Endangered or Threatened under the Endangered Species Act. The maps in this document record that decline and mark the strongholds where it's still holding on.
...
# Boundary and spatial layers from PostGIS
california = query_to_geodataframe("""
	SELECT geom
	FROM california_boundary
""", geom_col="geom", crs=4326)

frog_range = query_to_geodataframe("""
	SELECT "CName", "SName", geom
	FROM rana_boylii_range
""", geom_col="geom", crs=4326)

clades = query_to_geodataframe("""
	SELECT "Clade", geom
	FROM rana_boylii_clades
""", geom_col="geom", crs=4326)

ecoregions = query_to_geodataframe("""
	SELECT "US_L3NAME", geom
	FROM ca_ecoregions
""", geom_col="geom", crs=4326)

# Occurrences from PostGIS
occ_df = query_to_dataframe("""
	SELECT lat, lon, year, month
	FROM rana_boylii_occurrences
...
# Personal sighting — local GeoJSON
farm = gpd.read_file(str(FARM_PATH)).to_crs(epsg=4326)
...
**Connectivity.** Unlike many California frogs, *Rana boylii* does not make long overland migrations. But it does move within a drainage, upstream to spawn and downstream after breeding, and it needs those corridors unblocked by impassable barriers or completely unfavorable reaches.

## Range and Management Units
...
GBIF, the Global Biodiversity Information Facility, aggregates species observation records from museums, research institutions, and citizen science platforms into a single open database. The California records here span the 1890s to the present.
...
for _, row in eco_simple.iterrows():
	...
	folium.GeoJson(
		...
		tooltip=folium.Tooltip(
			f"<b style='font-family:sans-serif'>{name}</b>"
		),
	).add_to(eco_layer)
eco_layer.add_to(m)
...
heat_data = occ_df[["lat", "lon"]].dropna().values.tolist()
HeatMap(
	heat_data,
	min_opacity=0.3,
	radius=8,
...
sighting_layer = folium.FeatureGroup(
	name="Documented Sighting (Feather R. foothills)", show=True
)
folium.CircleMarker(
	...
	tooltip=folium.Tooltip(
		...
		"North Feather River DPS — "
		"<span style='color:#f59e0b; font-weight:700'>Threatened</span></span>"
		"</div>"
	),
...
<div style="margin-top:9px; margin-bottom:3px; font-size:10.5px; text-transform:uppercase;
            letter-spacing:0.04em; color:#5f7a6b;">Range &amp; Occurrences</div>
...
<div style="margin-top:9px; margin-bottom:3px; font-size:10.5px; text-transform:uppercase;
            letter-spacing:0.04em; color:#5f7a6b;">Management Clades (dashed)</div>
...
**Climate change.** The same hydrological sensitivities that make *Rana boylii* vulnerable to dam operations make it equally vulnerable to altered precipitation. More intense atmospheric river events followed by prolonged summer drought creates exactly the unpredictable flow regime that breaks the frog's life cycle: high water during egg development, early drying during tadpole development. Snow-fed foothill streams that once ran clear into August now dry weeks earlier as the snowpack retreats upslope.

**Water rights and environmental flow deficits.** California's rivers serve a long list of interests: irrigators who need summer deliveries, municipalities drawing from reservoirs, hydroelectric operators optimizing generation for peak-demand pricing. Environmental water requirements for *Rana boylii* are structurally at odds with most of these uses. The frog needs low, stable, gradually warming late-spring flows. Flow augmentation agreements negotiated under FERC hydroelectric licenses have produced real improvements on some drainages, including the North Fork Feather and parts of the Trinity. But enforcement mechanisms are weak and implementation is inconsistent year to year.
...
fig3 = go.Figure(go.Bar(
	...
	text=[f"{v}%" for v in dps_data["loss_pct"]],
...
occ_thin = gpd.GeoDataFrame(
	occ_df.iloc[::5].dropna(subset=["lat", "lon"]).copy(),
	geometry=gpd.points_from_xy(
		occ_df.iloc[::5]["lon"].dropna(),
		occ_df.iloc[::5]["lat"].dropna()
	),
	crs="EPSG:4326",
).to_crs(epsg=3310)
...
ax.set_title(
	"Rana boylii — California Range and Management Clades",
...
U.S. Fish and Wildlife Service. (2023–2024). *Listing of the Foothill Yellow-legged Frog as Threatened or Endangered Under the Endangered Species Act; Designation of Critical Habitat*. Federal Register, 88 FR 73498.

### Scholarly Sources

Welsh, H. H. Jr., Garthwaite, P. G., and Lind, A. J. (2016). Foothill yellow-legged frog (*Rana boylii*) oviposition site choice at multiple spatial scales. USDA Forest Service Pacific Southwest Research Station.
...
print(f"GBIF occurrences: {len(occ_gdf):,} points")
```

**Database:** `everglades_gis` (PostGIS). Tables: `california_boundary`, `rana_boylii_range`, `rana_boylii_clades`, `ca_ecoregions`, `rana_boylii_occurrences`.
**CRS:** EPSG:4326 (WGS84) for all spatial layers; EPSG:3310 (California Albers) for area calculations.
**Geometry simplification:** `simplify(0.005)` on range, clade, and ecoregion layers for Folium rendering performance.
**GBIF heatmap:** used instead of individual CircleMarkers to avoid Quarto serialization stack overflow on large point datasets.
